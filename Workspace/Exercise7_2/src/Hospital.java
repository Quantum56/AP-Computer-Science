...
public class Hospital {
	...
	public Hospital(Object[] doctorInfo, Object[] nurseInfo, Object[] administratorInfo, Object[] surgeonInfo, Object[] receptionistInfo, Object[] janitorInfo, String name0) throws Exception {
		name = name0;
		monthSwitch();
		Doctor doc = new Doctor(doctorInfo);
		Nurse nurse = new Nurse(nurseInfo);
		Administrator admin = new Administrator(administratorInfo);
		Surgeon surgeon = new Surgeon(surgeonInfo);
		Receptionist recep = new Receptionist(receptionistInfo);
		Janitor jan = new Janitor(janitorInfo);
		numTimes = patientTimes.size();
		while (numTimes > 0) {
			doc.examine(doc.getDoctorName(numTimes));
			nurse.assist(nurse.getNurseName(numTimes));
			admin.administrate(admin.getAdministratorName(numTimes));
			surgeon.operate(surgeon.getSurgeonName(numTimes));
			recep.manage(recep.getReceptionistName(numTimes));
			jan.sanatize(jan.getJanitorName(numTimes));
			numTimes -= 1;
		}
		try {
			super.finalize();
	...
	public class Doctor {
		public Doctor(Object[] doctorInfo) throws Exception {
			double time = 0;
	...
	public class Nurse {
		public Nurse(Object[] nurseInfo) throws Exception {
			double time = 0;
			String name = (String) null;
		...
		public void assist(String nurse) {
			if (nurses.indexOf(nurse) >= 0) {
				for (int i = 0; i < patientTimes.size(); i++) {
					...
					else if (patientTimes.get(i) == now.hashCode()
							&& patientTimes.indexOf(patientTimes.get(i)) % 3 == 0)
	...
	public class Administrator {
		public Administrator(Object[] administratorInfo) throws Exception {
			String name = (String) null;
	...
	public class Surgeon {
		public Surgeon(Object[] surgeonInfo) throws Exception {
			double time = 0;
			String name = (String) null;
	...
	public class Receptionist {
		public Receptionist(Object[] receptionistInfo) throws Exception {
			double time = 0;
			String name = (String) null;
		...
		public void manage(String receptionist) {
			...
			if (answer == "t" || answer == "T") {
				...
				System.out.println("Your appointment is in " + monthString + ", on the " + gen.nextInt(31)
						+ "th, and at " + time + ".");
	...
	public class Janitor {
		public Janitor(Object[] janitorInfo) throws Exception {
			String name = (String) null;
	...
	/*
	 * public String getDoctorName(int index) { return doctors.get(index); }
	 * 
	 * public String getNurseName(int index) { return nurses.get(index); }
	 * 
	 * public String getAdministratorName(int index) { return
	 * administrators.get(index); }
	 * 
	 * public String getSurgeonName(int index) { return surgeons.get(index); }
	 * 
	 * public String getReceptionistName(int index) { return
	 * receptionists.get(index); }
	 * 
	 * public String getJanitorName(int index) { return janitors.get(index); }
	 * 
	 * public void examine(String doctor) throws InterruptedException { if
	 * (doctors.indexOf(doctor) >= 0) { System.out.println("What is your name?");
	 * String name = sc.nextLine(); char first = name.charAt(0); String time =
	 * fmt.format(patientTimes.get(((int) first) % 10));
	 * System.out.println("Your appointment is at: " + time);
	 * System.out.println("Would you like to take the appointment?"); String answer
	 * = sc.nextLine(); if (answer.equals("YES") || answer.equals("Yes") ||
	 * answer.equals("yes") || answer.equals("y") || answer.equals("Y")) {
	 * System.out.println("Exercising physical"); for (int i = 0; i < 5; i++) {
	 * System.out.println(". "); Thread.sleep(20); } System.out.println("Done!"); }
	 * else { System.out.println("Goodbye!"); } } else System.out.println(doctor +
	 * ": \"I'm not a doctor, I can't examine patients!\""); }
	 * 
	 * public void assist(String nurse) { if (nurses.indexOf(nurse) >= 0) { for (int
	 * i = 0; i < patientTimes.size(); i++) { if (patientTimes.get(i) ==
	 * now.hashCode() && patientTimes.indexOf(patientTimes.get(i)) % 2 == 0)
	 * System.out.println("Helped nurse " + nurses.get(i / 2) + "!"); else if
	 * (patientTimes.get(i) == now.hashCode() &&
	 * patientTimes.indexOf(patientTimes.get(i)) % 3 == 0)
	 * System.out.println("Helped surgeon" + surgeons.get(i / 3) + "! "); else
	 * System.out.println("Helped doctor" + doctors.get(i) + "!"); } } }
	 * 
	 * public void administrate(String administrator) { System.out.println("At " +
	 * now.hashCode() + ", overseen " + doctors.get(now.hashCode() % 10) + ", " +
	 * nurses.get(now.hashCode() % 10) + ", " + surgeons.get(now.hashCode() % 10) +
	 * ", " + receptionists.get(now.hashCode() % 10) + ", " +
	 * janitors.get(now.hashCode() % 10) + "."); }
	 * 
	 * public void operate(String surgeon) {
	 * System.out.println("Entered the emergency room at " + now.hashCode() +
	 * ",  operating on" + patients.get(Clock.systemUTC().hashCode() % 10)); }
	 * 
	 * public void manage(String receptionist) { System.out.
	 * println("Would you like to add a patient time and name, or do you have an emergency? (t / e)"
	 * ); String answer = sc.nextLine(); if (answer == "t" || answer == "T") {
	 * System.out.println("Enter the patient's name here: "); String name =
	 * sc.nextLine(); patients.add(name);
	 * System.out.println("Enter the time of appointment: "); double time =
	 * Double.parseDouble(fmt.format(sc.nextDouble())); patientTimes.add(time);
	 * System.out.println("Your appointment is in " + monthString + ", on the " +
	 * gen.nextInt(31) + "th, and at " + time + "."); } else if (answer == "e" ||
	 * answer == "E") { System.out.println("Quickly, enter the emergency room!"); }
	 * }
	 * 
	 * public void sanatize(String janitor) {
	 * System.out.println("Cleaned floor at: " + now.hashCode() + "."); }
