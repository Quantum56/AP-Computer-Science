...
public class Queens {
	...
	private boolean isValid(LinkedList<Integer> state) {
		...
		/*
		 * We have to check for diagonal conflicts We do this by examining the "slope"
		 * of the last queen location on the stack against all the other queen
		 * positions. If the differences in x and y are equal, then the newest queen on
		 * the stack is in conflict with another queen.
		 */
	...
	private LinkedList<Integer> compute(LinkedList<Integer> state) {
		...
		for (int i = 0; i < this.n; i++) {
			...
			if (isValid(state)) {
				...
				// if the placement is valid, we need to try to place something in the next
				// column
				LinkedList<Integer> tmp = this.compute(state);
				if (tmp != null) {
					// if compute returns something non null, then we have reached a valid
					// configuration
